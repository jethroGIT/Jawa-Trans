...
export default function IndexUser() {
    ...
    // Fetch Data User on mount
    useEffect(() => {
        const loadUsers = async () => {
            try {
                setIsLoading(true);
                const data = await userMitraService.getAllUserMitra();
    ...
    const ActionButtons = ({ id }) => (
        <div className="flex items-center justify-center gap-2">
            <button
                onClick={() => navigate(`/admin/user-mitra/${id}`)}
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire('Info', 'Fitur hapus belum diimplementasikan di kode ini.', 'info');
            }
        });
    };
