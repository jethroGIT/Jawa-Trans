...
const update = async (req, res) => {
    ...
    const { idBus, titik_naik, titik_turun, tanggal_keberangkatan, jam_keberangkatan, tanggal_kedatangan, jam_kedatangan, harga } = req.body;
    try {
        const jadwal = await jadwalService.updateJadwal({ id, idBus, titik_naik, titik_turun, tanggal_keberangkatan, jam_keberangkatan, tanggal_kedatangan, jam_kedatangan, harga });
        return res.status(200).json({
            success: true,
